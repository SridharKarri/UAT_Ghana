...
public class MerchantDaoImpl implements MerchantDao {
	...
	@Override
	public StoreAccount getStoreAccountByCurrency(String Id, String currency) {
		Criteria criteria = null;
		StoreAccount store = null;
		Session session = null;
		try {
			session = sessionFactory.openSession();
			criteria = session.createCriteria(StoreAccount.class);
			criteria.add(Restrictions.eq("storeId", Id));
			criteria.add(Restrictions.eq("currency", currency));
			criteria.add(Restrictions.eq("status", Status.A.toString()));
			store = (StoreAccount) criteria.uniqueResult();
			log.debug("Store.." + store.toString());
