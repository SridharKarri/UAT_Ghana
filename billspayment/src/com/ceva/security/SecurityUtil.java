...
public class SecurityUtil {
	...
	public static String generateBodyHashString(String data)
	{
		Map<String, Object> map = getSortedMap(data);
		String strToHash = getSortedStringToHash(map);
		 try {
			MessageDigest md = MessageDigest.getInstance("SHA-256");
			byte[] dataInBytes = strToHash.getBytes(StandardCharsets.UTF_8);
			md.update(dataInBytes);
	...
	public static Map<String, Object> getSortedMap(String input){
		JSONObject data = JSONObject.fromObject(input);
	...
	public static String getAndroidPath(Map<Inputs, String> input,Inputs inp,byte[] key,byte[] iv)
	{
		...
		try{
			...
			String path = AESCBCEncryption.decrypt(key, siv, input.get(inp));
