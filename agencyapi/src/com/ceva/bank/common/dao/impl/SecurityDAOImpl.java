...
import com.ceva.security.FilterFactory;
...
public class SecurityDAOImpl {
	...
	public ChannelUsers prepareAppLogin(String userId, String imeiNo, String channelId, String serialNo, String macAddr, String version, String deviceIp,String language)
	{
		...
		try{
			...
			iosKey = fetchMasterKey(bundle.getString("ANDROID.CHANNEL"));
			...
			if(userChannel != null){
				...
				userChannel.setLanguage(language);
				...
				channelUser2.setLanguage(language);
	...
	{
		logger.info("preparing app login for...:"+userId);
		String key=null;
		String iosKey=null;
		String appId=null;
		String piv = null;
		...
		try{
			...
			if(userChannel != null){
				logger.debug("device found..:");
				userChannel.setCannelId("APP");
				userChannel.setUserLoginCredentials(credentials);
				userChannel.setStatus(Status.A.toString());
				userChannel.setpIv(piv);
				userChannel.setsIv(siv);
				userChannel.setpKey(key);
				//userChannel.setAppId(appId);
				//userChannel.setImeiNo(imeiNo);
				userChannel.setpToken(CommonUtil.generateToken());
				userChannel.setActiveLogin(new Long(0));
				userChannel.setSessionId(iosKey);
				userChannel.setLastRequestTime(new Date());
				userChannel.setDeviceIp(deviceIp);
				...
				channelUser2.setpKey(key);
				channelUser2.setAppId(appId);
				channelUser2.setpToken(CommonUtil.generateToken());
				channelUser2.setActiveLogin(new Long(0));
				channelUser2.setSessionId(iosKey);
				channelUser2.setMerchantId(credentials.getAgent());
				channelUser2.setStoreId(credentials.getStore());
				...
				channelUser2.setSerialNo(serialNo);
				channelUser2.setVersion(version);
				//credentials.getChannelUsers().add(channelUser2);
				channelUser2.setUserLoginCredentials(credentials);
				credentials.getChannelUsers().add(channelUser2);
				logger.debug(channelUser2.toString());
				//session.saveOrUpdate(channelUser);
	...
	/*public ChannelUsers prepareAppLogin(String userId, String imeiNo, String channelId) throws SQLException, ActiveUserRegistrationException
	{
	//changed for appid to imei dependency
		logger.info("preparing app login for...:"+userId);
		String key=null;
		String iosKey=null;
		String appId=null;
		String piv = null;
		String siv = null;
		IOSEnDesEncyp ios = new IOSEnDesEncyp();
		UserLoginCredentials credentials = null;
		Session session = null;
		ChannelUsers userChannel = null;
		try{
			//insert data into user channel
			key = AESCBCEncryption.base64Encode(AESCBCEncryption.generateSessionKey());
			appId = String.valueOf(System.nanoTime());
			siv = AESCBCEncryption.base64Encode(AESCBCEncryption.generateIV());
			piv = AESCBCEncryption.base64Encode(AESCBCEncryption.generateIV());
			...
			credentials = (UserLoginCredentials) session.createCriteria(UserLoginCredentials.class)
							...
							.add(Restrictions.in("status", new String[]{Status.A.toString(), Status.F.toString()})).uniqueResult();
							//.add(Restrictions.eq("status", Status.A.toString())).uniqueResult();
			logger.info("Loaded data..:"+credentials);
			if(credentials !=null){
				logger.debug("record found..");
				Set<ChannelUsers> channelSet = credentials.getChannelUsers();
				for(ChannelUsers channelUser : channelSet){
					logger.info("verifying channel id.."+channelId);
					if(channelId.equals(channelUser.getCannelId())){
						boolean isActiveChannel = isUserActive(new Date(), channelUser.getLastRequestTime());
						logger.debug("isActiveChannel..:"+isActiveChannel);
						if(!isActiveChannel){
							ChannelUsers channelUser2= new ChannelUsers();
							channelUser2.setCannelId("APP");
							channelUser2.setUserLoginCredentials(credentials);
							channelUser2.setCannelValue(channelUser.getCannelValue());
							channelUser2.setStatus(Status.A.toString());
							channelUser2.setpIv(piv);
							channelUser2.setsIv(siv);
							channelUser2.setpKey(key);
							channelUser2.setAppId(appId);
							channelUser2.setpToken(CommonUtil.generateToken());
							channelUser2.setActiveLogin(new Long(1));
							channelUser2.setSessionId(iosKey);
							channelUser2.setMerchantId(channelUser.getMerchantId());
							channelUser2.setStoreId(channelUser.getStoreId());
							channelUser2.setImeiNo(imeiNo);
							//session.saveOrUpdate(channelUser);
							session.save(channelUser2);
							userChannel=channelUser2;
							...
							logger.debug("the user is an active user, we can able to register channel at this moment.");
							throw new ActiveUserRegistrationException("Unable to Register channel for an Active User");
						}
					}
				}
				session.flush();
				...
				logger.debug("No User Found with the userId."+userId);
				throw new NullPointerException("Wrong User ID OR Pin");
			}

			}catch(ActiveUserRegistrationException e){
			//e.printStackTrace();
				logger.debug("Unable to Register channel for an Active User..:"+userId);
			throw new ActiveUserRegistrationException("Unable to Register channel for an Active User");
		}
		catch(NullPointerException e){
			//e.printStackTrace();
			logger.debug("Unable to Process your Request.:"+userId);
			throw new SQLException("Wrong User ID OR Pin");
		}
		catch(Exception e){
			//e.printStackTrace();
			throw new SQLException("Wrong User ID OR Pin");
