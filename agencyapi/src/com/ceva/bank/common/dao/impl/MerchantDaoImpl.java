...
public class MerchantDaoImpl implements MerchantDao{
	...
	@Override
	public boolean updateStoreLocation(UserLocation userLocation) {
		log.debug("updateStoreLocation..:START");
		boolean isUpdated =false;
		Session session =null;
		try {
			session = sessionFactory.openSession();
			Query query = session.createQuery("update Store set latitude = :latitude, longitude=:longitude" +
    				" where storeId = :storeId");
			query.setParameter("latitude", userLocation.getLatitude());
			query.setParameter("longitude", userLocation.getLongitude());
			query.setParameter("storeId", userLocation.getStoreId());
			int result = query.executeUpdate();
			if(result > 0 )
				isUpdated = true; 
		} catch (Exception e) {
			log.error("Error while update storeLoaction..:" + e.getLocalizedMessage());
		} finally {
			session.close();
		}
		log.debug("updateStoreLocation..:END");
		return isUpdated;
	}
