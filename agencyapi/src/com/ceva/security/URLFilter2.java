...
public class URLFilter2 implements Filter,Modes{
	ServletContext context;
	private SecurityDAOImpl securityDAO;
	private SecurityUtil securityUtil;
	private static Logger logger = Logger.getLogger(URLFilter2.class);
	...
	public Map<Inputs,String> splitInput(String uri) throws UnsupportedEncodingException, DecoderException{
		...
		Integer i=0;
		logger.debug("splitInput Length : "+inputs.length+"]");
		if(inputs.length != 17)
			return null;
		//if(inputs.length != 16)
		for(Inputs eInp : Inputs.values()){
			...
			//logger.debug(inputs[i]+": = :"+eInp);
			i++;
		}
		//logger.debug("OUTPUT MAP : "+inpMap);
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		logger.debug("URL endpoint : "+urlEndPoint);
		logger.debug("HIT : "+ Arrays.asList(Constants.ANDROID_END_POINTS).indexOf(urlEndPoint));
		...
		if(inputMap != null && Arrays.asList(Constants.ANDROID_END_POINTS).indexOf(urlEndPoint.substring(0, urlEndPoint.indexOf("."))) > -1 && queryString == null)
		{
			...
			if(urlEndPoint.equalsIgnoreCase("login.action") || urlEndPoint.equalsIgnoreCase("devReg.action")|| urlEndPoint.equalsIgnoreCase("generateotp.action")|| urlEndPoint.equalsIgnoreCase("echo"))
				...
				case ANDROID:
					...
					if(appid.equals(bundle.getString("APPID"))){
						logger.debug("identified device registration..");
						String path=null;
						...
						try{
							...
							if(securityDAO.noteFraud(SecurityUtil.buildErrorMap(newRequest, "Unable to Decrypt the request parameters sent for First login.", bundle.getString("ANDROID.CHANNEL"), appid))){
								logger.debug("[URLFilter][filter][ENCRYPT][FRAUD FOUND]");
							}
							HttpServletResponse resp = (HttpServletResponse) response;
							resp.reset();
						...
						try{
							if("devReg.action".equals(urlEndPoint)){
								//bean = securityDAO.prepareAppLogin(p[1],p[3],"APP");
								bean = dummyChannelUser();
								logger.debug("prepareAppLogin response : "+path+"\nBean : "+bean);
							}else{
								logger.debug("Identified otp for device registration..!");
								bean = dummyChannelUser();
							}
						...
						{
							if(securityDAO.noteFraud(SecurityUtil.buildErrorMap(newRequest, "Digital Signature Mismatched for First login", bundle.getString("ANDROID.CHANNEL"), appid))){
								//logger.info("[URLFilter][filter][ENCRYPT][FRAUD FOUND]");
							}
							logger.info("[URLFilter][filter][DH][Signature ERROR]");
							isFraud=true;
							HttpServletResponse resp = (HttpServletResponse) response;
							resp.reset();
							resp.setStatus(HttpServletResponse.SC_BAD_REQUEST);
							resp.setHeader("Location", "");
							logger.error("Hash mimatch please check ");
							//return;
						}
						...
						logger.debug("passed all security checks.");
						...
						try{
							...
							logger.debug("loaded security data..:");
							...
							e.printStackTrace();
							if(securityDAO.noteFraud(SecurityUtil.buildErrorMap(newRequest, "Unable to Decrypt the request parameters sent for General login", bundle.getString("ANDROID.CHANNEL"), appid))){
								logger.info("[URLFilter][filter][ENCRYPT][FRAUD FOUND]");
							}
							HttpServletResponse resp = (HttpServletResponse) response;
							resp.reset();
						...
						if(!hashCode.equals(SecurityUtil.generateHashString(path)))
						{
							if(securityDAO.noteFraud(SecurityUtil.buildErrorMap(newRequest, "Digital Signature Mismatched for General login", bundle.getString("ANDROID.CHANNEL"), appid))){
								logger.debug("[URLFilter][filter][ENCRYPT][FRAUD FOUND]");
							}
							HttpServletResponse resp = (HttpServletResponse) response;
							resp.reset();
						...
						if(enhancedHttpRequest==null){
							if(securityDAO.noteFraud(SecurityUtil.buildErrorMap(newRequest, "Unable to build the request object for General login", bundle.getString("ANDROID.CHANNEL"), appid))){
								logger.debug("[URLFilter][filter][ENCRYPT][FRAUD FOUND]");
							}
							HttpServletResponse resp = (HttpServletResponse) response;
							resp.reset();
			...
			else
			{
				...
				logger.debug("Transaction sec ................");

				ChannelUsers bean=null;
				logger.debug("CHKEY.:"+inputMap.get(Inputs.CHKEY));
				switch(inputMap.get(Inputs.CHKEY))
				{
				//case ANDROID:	synchronized (this) {
				case ANDROID:
					...
					try{
						...
						if(!hashCode.equals(SecurityUtil.generateHashString(path)))
						{
							if(securityDAO.noteFraud(SecurityUtil.buildErrorMap(newRequest, "Digital Signature Mismatched for "+inputMap.get(Inputs.ENDPOINT), bundle.getString("ANDROID.CHANNEL"), appid))){
								logger.info("[URLFilter][filter][ENCRYPT][FRAUD FOUND]");
							}
							HttpServletResponse resp = (HttpServletResponse) response;
							resp.reset();
						...
						logger.info("[UrlFilter][Filter][Real Path : "+path+"]");
						String[] p = path.split("/");
						logger.debug("[UrlFilter][Filter][BEAN : "+bean.getpToken()+"  "+ bean.getpId()+"]");
						...
						if(enhancedHttpRequest==null){
							if(securityDAO.noteFraud(SecurityUtil.buildErrorMap(newRequest, "Unable to build the request object for action "+inputMap.get(Inputs.ENDPOINT), bundle.getString("ANDROID.CHANNEL"), appid))){
								logger.info("[URLFilter][filter][ENCRYPT][FRAUD FOUND]");
							}
							HttpServletResponse resp = (HttpServletResponse) response;
							resp.reset();
						...
						e.printStackTrace();
						if(securityDAO.noteFraud(SecurityUtil.buildErrorMap(newRequest, "Unable to Decrypt the request parameters sent for the Tranaction "+inputMap.get(Inputs.ENDPOINT), bundle.getString("ANDROID.CHANNEL"), appid))){
							logger.error("[URLFilter][filter][ENCRYPT][FRAUD FOUND]");
						}
						HttpServletResponse resp = (HttpServletResponse) response;
						resp.reset();
						resp.setStatus(HttpServletResponse.SC_NOT_FOUND);
		...
		else
		{
			...
			if(securityDAO.noteFraud(SecurityUtil.buildErrorMap(newRequest, "Invalid Endpoint found.", null, null))){
				logger.info("[URLFilter][filter][ENCRYPT][FRAUD FOUND]");
			}
			HttpServletResponse resp = (HttpServletResponse) response;
			resp.reset();
